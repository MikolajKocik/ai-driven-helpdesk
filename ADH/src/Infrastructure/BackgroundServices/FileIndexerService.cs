...
public class FileIndexerService : BackgroundService
{
    ...
    protected override Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        _watcher.Created += async (s, e) => await IndexFileAsync(e.FullPath);
        _watcher.Changed += async (s, e) => await IndexFileAsync(e.FullPath);
        
        _watcher.EnableRaisingEvents = true;

        _ = Task.Run(() => InitialScan(path), stoppingToken);

        return Task.CompletedTask;
    }

    private async Task InitialScan(string path)
    {
    ...
    private async Task IndexFileAsync(string filePath)
    {
        try
        {
            ...
            ITextEmbeddingGenerationService embeddingService = scope.ServiceProvider.GetRequiredService<ITextEmbeddingGenerationService>();
            ...
            for (int i = 0; i < chunks.Count; i++)
            {
                ...
                ReadOnlyMemory<float> embedding = await embeddingService.GenerateEmbeddingAsync(chunkText);
